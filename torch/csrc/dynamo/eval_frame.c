...
static PyObject* nn_module_map = NULL;
...
inline PyObject* get_self(THP_EVAL_API_FRAME_OBJECT* frame) {
  PyObject* self_string = PyUnicode_FromString("self");
...
static inline PyObject* nn_module_cache_key(THP_EVAL_API_FRAME_OBJECT* frame) {
  char* prefix = "__dynamo_cache_";
  char* suffix = "__";
  const char* method_name = name(frame);

  int newSize = strlen(prefix) + strlen(suffix) + strlen(method_name) + 1;
  char * newBuffer = (char *)malloc(newSize);

  strcpy(newBuffer, prefix);
  strcat(newBuffer, method_name);
  strcat(newBuffer, suffix);
  fprintf(stderr, "Checking %s\n", newBuffer);
  return PyUnicode_FromString(newBuffer);
...
// static PyModuleDef cache_entry_module = {
...
// PyMODINIT_FUNC
// PyInit_CacheEntryPyWrapper(void)
// {
//     PyObject *m;
//     if (PyType_Ready(&CacheEntryWrapperType) < 0)
//         return NULL;

//     m = PyModule_Create(&cache_entry_module);
//     if (m == NULL)
//         return NULL;

//     Py_INCREF(&CacheEntryWrapperType);
//     if (PyModule_AddObject(m, "CacheEntryPyWrapper", (PyObject *) &CacheEntryWrapperType) < 0) {
...
inline static CacheEntry* get_cache_entry_from_module(THP_EVAL_API_FRAME_OBJECT* frame) {
    ...
    PyObject* cache_key = nn_module_cache_key(frame);
    PyObject* nn_module_dict = PyObject_GetAttr(nn_module, PyUnicode_FromString("__dict__"));


    if (PyDict_GetItem(nn_module_dict, cache_key) == NULL)  {
      // TODO - Move the PyType_Ready to some place else. Is this somehow related to PyMODINIT_FUNC
      PyType_Ready(&CacheEntryWrapperType);
      PyObject *cache_entry_wrapper = PyObject_CallObject((PyObject *) &CacheEntryWrapperType, NULL);
      PyDict_SetItem(nn_module_dict, cache_key, cache_entry_wrapper);
    }
    CacheEntryPyWrapper* cache_entry_wrapper = (CacheEntryPyWrapper*)PyDict_GetItem(nn_module_dict, cache_key);
...
inline static void set_cache_entry(THP_EVAL_API_FRAME_OBJECT* frame, CacheEntry* extra) {
  // TODO(jansel): would it be faster to bypass this?
  if (is_nn_module_method(frame)) {
    CacheEntry *e = get_cache_entry_from_module(frame);
    e = extra;
    return;
  }
...
static PyObject* lookup(CacheEntry* e, THP_EVAL_API_FRAME_OBJECT *frame, CacheEntry* prev, size_t index) {
  fprintf(stderr, "Address of cache entry: %p\n", ( void * )e );
  if (e == NULL) {
